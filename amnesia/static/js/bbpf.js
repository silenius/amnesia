;(function(ns) {
    ...
    **************************************************************************/
    ...
    function _browser_callback(field_name, url, type, win) {
        ...
        /* Build the frame window and the browser object */
        ...
        if (type == 'image') {
            browser.parameters.filter_content_type = ['folder'];
            ...
            for (var i=0, _len = select.length; i<_len; i++) {
                (function(elem) {
                    elem.addEventListener('click', function(e) {
                        ...
                        if (type == 'image') {
                            field.value = obj_meta.id + '/download/inline';
                        } else if (type == 'file') {
                ...
                (function(elem) {
                    elem.addEventListener('click', function(e) {
                        ...
                        if (obj_meta.type.name == 'folder') {
                            browser.url = site_url('folder/browse', obj_meta.id);
                            browser.load();
    ...
    Bbpf.utils.init_tinyMCE = function(params) {
        ...
        var default_values = {
            ...
            content_css : site_url('styles/_mce.css'),
            style_formats : [
                {title : 'Table styles'},
                {title : 'with borders', selector : 'table', classes : 'with_borders'}
            ],
...
//            relative_urls : false,
    ...
    Bbpf.utils.form_tabs = function(selector, current) {
        var css_selector = '[' + selector + ']',
            lis = document.querySelectorAll('li' + css_selector),
            divs = document.querySelectorAll('div' + css_selector)
        ;

        for (var i=0, _len = lis.length; i<_len; i++) {
            (function(elem) {
                var section = elem.getAttribute(selector);
                if (section) {
                    elem.addEventListener('click', function() {
                        for (var i=0, _len = divs.length; i<_len; i++) {
                            var div = divs[i];
                            if (div.getAttribute(selector) == section) {
                                Yeti.DOM.removeClass(div, 'hidden');
                            } else {
                                Yeti.DOM.addClass(div, 'hidden');
                            }
                        }

                        for (var i=0, _len = lis.length; i<_len; i++) {
                            var li = lis[i];
                            if (li.getAttribute(selector) == section) {
                                Yeti.DOM.addClass(li, 'current');
                            } else {
                                Yeti.DOM.removeClass(li, 'current');
                            }
                        }
    ...
    **************************************************************************/
    ...
    Bbpf.Spinner = function(params) {
        this.target = Yeti.Element(params.target);
        this.images = params.images;
    }

    Bbpf.Spinner.prototype.set = function(action, show) {
        var src = this.images[action];
        if (src && this.target.src != src) {
            this.target.src = src;
        }
    ...
    Bbpf.Spinner.prototype.show = function() {
        this.target.style.display = 'inline';
    }

    Bbpf.Spinner.prototype.hide = function() {
        this.target.style.display = 'none';
    }

    Bbpf.Spinner.prototype.toggle = function() {
    ...
    **************************************************************************/
    ...
    Bbpf.Clipboard = function(params) {
        ...
        // Trash icon (to empty the clipboard)
        this._trash_img = document.createElement('img');
        this._trash_img.src = this.images.trash;
        this._trash_img.className = 'trash_empty';
        this._trash_img.addEventListener('click', function() {
            _self.empty();
        })

        // The <span> element
        this._cpt = document.createElement('span');

        this.container.appendChild(this._img);
        this.container.appendChild(this._cpt);
        this.container.appendChild(this._trash_img);

        this.update(params.ids);
    }

    Bbpf.Clipboard.prototype.update = function(ids) {
        this.ids = ids;
        var _count = this.count();

        Yeti.DOM.removeNodes(this._cpt);
        this._cpt.appendChild(document.createTextNode('(' + _count + ')'));

        if (_count > 0) {
            this._img.src = this.images.filled;
            Yeti.DOM.removeClass(this._img, 'clipboard_empty');
            Yeti.DOM.addClass(this._img, 'clipboard_count');

            Yeti.DOM.removeClass(this._trash_img, 'trash_empty');
            Yeti.DOM.addClass(this._trash_img, 'trash_full');
        } else {
            this._img.src = this.images.empty;
            Yeti.DOM.removeClass(this._img, 'clipboard_count');
            Yeti.DOM.addClass(this._img, 'clipboard_empty');

            Yeti.DOM.removeClass(this._trash_img, 'trash_full');
            Yeti.DOM.addClass(this._trash_img, 'trash_empty');
        }
    }

    Bbpf.Clipboard.prototype.count = function() {
        return this.ids.length;
    }

    Bbpf.Clipboard.prototype.empty = function() {
        var _self = this;
        Yeti.AjaxRequest(site_url('content/sremove'), {
            method : 'GET',
            onreadystatechange : function(req) {
                if (req.success()) {
                    _self.clear()
                    _self.dispatcher.fire('after_empty_clipboard_success');
                }
            }
        });
    }

    Bbpf.Clipboard.prototype.clear = function() {
        this.update([]);
    }
    ...
    **************************************************************************/
    ...
    Bbpf.TagList.prototype.load = function() {
        ...
        Yeti.AjaxRequest(this.url, {
            ...
            'onreadystatechange' : function(req) {
                if (req.success()) {
                    var data = Yeti.JSON.parse(req.get_response());
                    Yeti.DOM.removeNodes(_self.container);
    ...
    **************************************************************************/
    ...
    Bbpf.Tag.prototype.save = function(params) {
        ...
        Yeti.AjaxRequest(site_url('tag/save'), {
            method : 'POST',
            accept : 'json',
            data : _self.data,
            onreadystatechange : function(req) {
                if (req.success()) {
                    var tag_json = Yeti.JSON.parse(req.get_response());
    ...
    **************************************************************************/
    ...
    Bbpf.PaginationComponent.prototype.add_handlers = function() {
        ...
        // Special case for the input .. a little dirty
        links = this.container.getElementsByTagName('input');
        for (var i=0, _len=links.length; i<_len; i++) {
            (function(el) {
                if (el.getAttribute('data-direction') == 'goto') {
                    el.addEventListener('change', function() {
                        _self.pagination.page(this.value);
                    });
                }
            })(links[i]);
        }
    ...
    **************************************************************************/
    ...
    Bbpf.FolderController.prototype.load = function() {
        ...
        if (this.sort) { 
            ...
            this.sort.forEach(function(sort, cpt) {
                ...
                _self.parameters[s + 'dir'] = sort.dir
    ...
    Bbpf.FolderController.prototype.move = function(params) {
        if (!this.selection_move) {
            ...
            Yeti.DOM.addClass(params.container, 'move_select');
        } else if (this.selection_move == params.container) {
            delete(this.selection_move);
            Yeti.DOM.removeClass(params.container, 'move_select');
        } else {
            Yeti.DOM.addClass(params.container, 'move_do');
    ...
    **************************************************************************/
    ...
    Bbpf.Content.delete_objs = function(selection, dispatcher) {
        if (selection.length > 0) {
            Yeti.AjaxRequest(site_url('content/delete'), {
                method : 'POST',
                accept : 'json',
                data : {
                    id : selection
    ...
    Bbpf.Content.copy_objs = function(selection, dispatcher) {
        if (selection.length > 0 ) {
            Yeti.AjaxRequest(site_url('content/scopy'), {
                method : 'POST',
                accept : 'json',
                data : {
                    id : selection
    ...
    Bbpf.Content.change_weight = function(p) {
        Yeti.AjaxRequest(site_url('content/weight', p.from.obj.id, 
                                  p.to.obj.weight), {
            accept: 'json',
            onreadystatechange : function(req) {
                if (req.success()) {
    ...
    **************************************************************************/
    ...
    Bbpf.Folder.prototype.paste = function(params) {
        if (confirm('Are you sure that you want to move those items ? They will me moved from their original location !')) {
            var _self = this;
            Yeti.AjaxRequest(site_url('folder/paste', this.id), {
                onreadystatechange : function(req) {
                    if (req.success()) {
    ...
    **************************************************************************/
    ...
    Bbpf.Page = function(params) {
        Bbpf.Content.call(this, params);
    }
    Bbpf.Page.prototype = new Bbpf.Content();
    Bbpf.Page.prototype.constructor = Bbpf.Page;

    /*************************************************************************
        News
    **************************************************************************/

    Bbpf.News = function(params) {
        Bbpf.Content.call(this, params);
    }
    Bbpf.News.prototype = new Bbpf.Content();
    Bbpf.News.prototype.constructor = Bbpf.News;

    /*************************************************************************
        Platform News
    **************************************************************************/

    Bbpf.PlatformNews = function(params) {
        Bbpf.News.call(this, params);
    }
    Bbpf.PlatformNews.prototype = new Bbpf.News();
    Bbpf.PlatformNews.prototype.constructor = Bbpf.PlatformNews;

    /*************************************************************************
        Science News
    **************************************************************************/

    Bbpf.ScienceNews = function(params) {
        Bbpf.News.call(this, params);
    }
    Bbpf.ScienceNews.prototype = new Bbpf.News();
    Bbpf.ScienceNews.prototype.constructor = Bbpf.ScienceNews;

    /*************************************************************************
        NewsFlash
    **************************************************************************/

    Bbpf.NewsFlash = function(params) {
        Bbpf.PlatformNews.call(this, params);
    }
    Bbpf.NewsFlash.prototype = new Bbpf.PlatformNews();
    Bbpf.NewsFlash.prototype.constructor = Bbpf.NewsFlash;
